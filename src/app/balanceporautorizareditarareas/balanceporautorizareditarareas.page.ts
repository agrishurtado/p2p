...
@Component({
  selector: 'app-balanceporautorizareditarareas',
  templateUrl: './balanceporautorizareditarareas.page.html',
  styleUrls: ['./balanceporautorizareditarareas.page.scss'],
})
export class BalanceporautorizareditarareasPage implements OnInit {
...
// registros de areas comunes de administrador 
async getPostBy(id: string) {
  ...
  this.firestore.doc("pagoareascomunesadmin/" + id)
    .valueChanges()
    .subscribe(data => {
      this.id
...
async updatePost(post: Autorizadoz){
  if(this.formValidation()){
    ...
    try {
      ...
      await this.firestore.doc("pagoareascomunesadmin/" + this.id).update(post);     
    } catch (e) {
      this.showToast(e);
    }
    //cerrar loading
...
// metodo para archivar los  pagos autorizados o rechazados 
  ...
  async deletePost(id: string) {
    ...
    await this.firestore.doc("pagoareascomunesadmin/" + id).delete();
    (await loader).dismiss();
  }
// validaciones campos 
  formValidation() {
